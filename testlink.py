...
# 读取 JSON 文件
with open('D:\\Wanji\\济南绿波城项目\\高德数据接入\\2024-11-15_20-07-24_link_info.json', 'r', encoding='utf8') as file:
    data = json.load(file)
features=[]
# 遍历 JSON 数组并打印每个元素
for item in data['linkCoordList']:
    linkId = item['linkId']
    coordinates = item['coordList']
    roadName= item['roadName']
    length= item['length']
    formway= item['formway']
    ...
    features.append({
                ...
                "geometry": {
                    "type": "LineString",
                    "coordinates": [ Geocoding().GCJ02_to_WGS84(lon, lat) for lon, lat in convert_to_2d_array(coordinates,2)]
                },
                "properties": {
                    "forwardroadid64": linkId,
                    "name_chn": roadName,
                    "length": length,
                    "form_way": formway,
...
print(geojson)
# 将数据写入 JSON 文件
with open('D:\\Wanji\\济南绿波城项目\\高德数据接入\gaode_link_new.json', 'w') as json_file:
