...
def remove_offsets(path: Route, p_off: float, n_off: float) -> Route:
    ...
    # log.debug(f"Will consider positive offset = {p_off} m and negative offset {n_off} m.")
    lines = path.lines
    # log.debug("result, {}".format('-' * 40))
    # log.debug(f"This routes consists of {lines} and is {path.length()} m long.")
    # Remove positive offset
    # log.debug(f"first line-s offset is {path.absolute_start_offset}")
    # log.debug(f"last line-s offset is -{path.absolute_end_offset}")

    remaining_poff = p_off + path.absolute_start_offset
    while remaining_poff >= lines[0].length:
        # log.debug(f"Remaining positive offset {remaining_poff} is greater than "
    ...
    while remaining_noff >= lines[-1].length:
        # log.debug(f"Remaining negative offset {remaining_noff} is greater than "
