...
def score_geolocation(wanted: LocationReferencePoint, actual: PointOnLine, radius: float) -> float:
    ...
    # log.debug(f"Candidate coords are {actual.position()}")
...
def score_lrp_candidate(
    ...
    # log.debug(f"scoring {candidate} with config {config}")
    ...
    #log.debug(f"Score: geo {geo_score} + fow {fow_score} + frc {frc_score} "
     #     f"+ bear {bear_score} = {score}")
    print(f"Score: geo {geo_score} + fow {fow_score} + frc {frc_score} "
          f"+ bear {bear_score} = {score}")
    return score
